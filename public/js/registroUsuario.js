...
`;
    ...
    document.querySelector('input[name="asignaciones[]"]').value = JSON.stringify(asignacionesPendientes);
}
...
function editarAsignacion(botonEditar) {
    ...
    asignacionesPendientes = asignacionesPendientes.filter(asignacion => asignacion.materia !== materiaId || asignacion.grupo !== grupo);
    document.querySelector('input[name="asignaciones[]"]').value = JSON.stringify(asignacionesPendientes);
}
